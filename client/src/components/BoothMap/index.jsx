...
import { MdHearing, MdOutlineInterpreterMode } from "react-icons/md";
...
function BoothMap() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      setData(sorted);
      setMarkers(
        sorted.filter((_, index) => (index + 1) % 10 === 0).splice(0, 100)
      );
      console.log(sorted);
    }
    fetchData();
    handleAudio(
      "On top left you'll find return button, bottom right,  you'll find utilities like AI Sahayak"
    );
  }, []);
  ...
  return (
    <div className="overflow-hidden">
      <FaChevronCircleLeft
        className="fixed left-8 top-8 z-50 cursor-pointer text-4xl"
        onClick={() => navigate(-1)}
      />
      <APIProvider apiKey={API_KEY} libraries={["marker"]}>
        <Map
          ...
          defaultZoom={12}
          defaultCenter={{ lat: 19.039, lng: 72.8619 }}
                ...
                if (marker.id === selectedId) {
                  return (
                    <div className="mx-auto">
                      ...
                      <br />
                      <ul className="mx-auto flex gap-4 text-3xl">
                        <li className="text-md">
                          {marker.braille && <BiBraille />}
                        </li>
                        <li className="text-md">
                          {marker.hearingAid && <MdHearing />}
                        </li>
                        <li className="text-md">
                          {marker.interpreter && <MdOutlineInterpreterMode />}
                        </li>
                      </ul>
                    </div>
                  );
